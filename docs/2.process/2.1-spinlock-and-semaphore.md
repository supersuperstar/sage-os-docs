...
tasksem=>condition: 检查该线程的wait_sem是否为当前的信号量（是否需要唤醒）
...
tasksem(no)->tasklist
checkstate(yes)->stateup->setsem->tasklist
