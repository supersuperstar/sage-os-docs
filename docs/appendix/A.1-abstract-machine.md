# 附录二：Abstract Machine

在本次实验中，我们使用了 Abstract Machine 硬件抽象层来实现操作系统[^1]。

AbstractMachine 解决的问题是 “能否在不理解硬件机制细节的前提下实现操作系统”。它旨在把硬件提供的机制 (可以看成是 “指令”) 象成一组 C API，并将这些 API 分为几组 (extensions)。在其中不可避免地丢失了一些对计算机系统的细节控制，但却屏蔽了琐碎的底层细节，帮助初学者理解计算机系统的本质。
...
- `main` 运行时带有一个参数，允许程序启动时传递一个字符串参数：
...
从编号为 `reg` 的寄存器读取/写入，读取/写入的数据取决于寄存器的编号。设备可能会对寄存器的使用作出额外的规定，请参考 [Abstract Machine 设备文档](http://jyywiki.cn/OS/AbstractMachine/AM_Devices)。为了减少大家定义额外的变量，我们推荐使用 klib-macros.h 中封装后的宏访问设备，例如：
...
对地址空间 `as` 建立 `vaddr` → `paddr` 的映射，当 `paddr` 为 `NULL` 时，取消该页面的映射。prot 由以下 flag 组成。注意 `vaddr` 和 `paddr` 必须对齐到页面边界 (`as->pgsize`)，且 `vaddr` 必须位于 `as->area`、`paddr` 指向的页面位于 `heap`。
...
[^1]: [NJU-ProjectN/abstract-machine: A minimal, modularized, and machine-independent hardware abstraction layer (github.com)](https://github.com/NJU-ProjectN/abstract-machine)
