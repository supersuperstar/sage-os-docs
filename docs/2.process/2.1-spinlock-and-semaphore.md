...
sem_wait 是一个函数，也需要是一个原子操作（所以应用了部分自旋锁），它的作用是从信号量的值减去一个“1”，但它永远会先等待该信号量为一个非零值才开始做减法：
...
同 sem_wait，该函数也需要大量原子操作，他的作用是将信号量的值加"1"。但因为节省资源需要，我们将等待该信号量值非零的线程全部沉睡，因此我们需要同时唤醒所有因为该信号量而沉睡的线程，竞争信号量的自旋锁，赢家得到使信号量的值减”1"的权利。
...
cursem=>operation: cur->wait_sem为当前信号量
...
semulock1=>operation: 释放当前信号量自旋锁
checktask=>condition: 是否持有线程链表自旋锁
tasklock=>operation: 申请线程链表自旋锁
tasklist=>condition: 链表是否遍历完毕
tasksem=>condition: wait_sem是否为当前的信号量
checkstate=>condition: state是否为睡眠状态
stateup=>operation: 唤醒该线程
setsem=>operation: 设置wait_sem为null
