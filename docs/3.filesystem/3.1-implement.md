...
文件系统分为5层实现，如上图所示。
最下面一层负责对硬盘中存储的dinode与block进行读写，并负责block的管理和分配。上一层的inode层提供无名文件，每一个这样的文件由一个i节点和一连串的数据块组成。第三层将目录实现为一种特殊的i节点，它的内容是一连串的目录项，每一个目录项包含一个文件名和对应的i节点。第四层提供了层次路经名(如/usr/rtm/sage-os/fs.c)，这一层通过递归的方式来查询路径对应的文件。最上层将各种不同类资源(设备，文件等)抽象为文件系统的接口。
...
每个目录都与一个i节点相对应，该i节点存储目录中所包含的目录项。

### 3.1.3.1 数据结构

#### dirent

存储在内存中的dirent目录由数据结构``dirent``定义，详见3.1.1.3
